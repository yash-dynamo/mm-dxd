...
const normalizeAgentSetupError = (err: unknown): string => {
  ...
  if (
    ...
    return 'Wallet/agent linkage check failed on backend. Update/restart backend service, then retry.';
  }
    ...
    async (agentName: string) => {
      ...
      try {
        ...
        if (!result?.success) {
          ...
          showToast.warning({
            message: 'Linkage check skipped',
            description:
              'Continuing setup without backend linkage verification due service mismatch.',
          });
        }
        if (!brokerAddress) {
